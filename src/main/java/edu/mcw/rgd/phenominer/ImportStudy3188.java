...
public class ImportStudy3188 extends ImportCommon {
    ...
    Condition parseCondition(int condCol, String[] cols) throws Exception {
        ...
        // sanity checks
        if( condDurLow.toUpperCase().equals("NA") || condDurLow.toUpperCase().equals("N/A") ) {
            condDurLow = "";
        }
        if( condDurHigh.toUpperCase().equals("NA") || condDurHigh.toUpperCase().equals("N/A") ) {
            condDurHigh = "";
        }
        if( condDurUnit.equals("na") || condDurUnit.equals("n/a") ) {
            condDurUnit = "";
        }
        if( condValue.toUpperCase().equals("NA") || condValue.toUpperCase().equals("N/A") ) {
            condValue = "";
        }
    ...
    boolean loadAsIndividualData() throws Exception {
        ...
        fname = "../resources/study3188_16w.txt";
        ratColPrefix = "rat";
        if( !loadAsIndividualData(fname, ratColPrefix) ) {
            return false;
        }

        fname = "../resources/study3188_17w.txt";
        ratColPrefix = "rat";
        if( !loadAsIndividualData(fname, ratColPrefix) ) {
            return false;
        }

        fname = "../resources/study3188_24w.txt";
        ratColPrefix = "Rat";
        if( !loadAsIndividualData(fname, ratColPrefix) ) {
            return false;
        }
    ...
    boolean loadAsIndividualData(String fname, String ratColPrefix) throws Exception {
        ...
        while( (line=bw.readLine())!=null ) {
            ...
            // determine start/end col for individual data
            if( row==2 ) {
                int[] range = determineIndividualDataCols(ratColPrefix, cols);
                indDataStartCol = range[0];
                indDataEndCol = range[1];
                continue;
            }

            // load animal ids
            if( row==3 ) {
                for( int x=indDataStartCol; x<=indDataEndCol; x++ ) {
                    String val = cols[x];
            ...
            String cmoId = cols[7];
            String units = cols[8];
            String mmoId = cols[indDataEndCol+3];
            String xcoId = cols[indDataEndCol+5];
            ...
            Condition cond = parseCondition(indDataEndCol+4, cols); // skip notes, MMO name and MMO id columns that are right after ind data section
    ...
    int[] determineIndividualDataCols(String prefix, String[] cols) {
    ...
    /* old code
        ...
        String fname = "../resources/study3188_24.txt";
        BufferedReader bw = Utils.openReader(fname);
    ...
    boolean loadAsIndividualDataOld() throws Exception {
